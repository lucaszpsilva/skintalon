...
export const Items = ({ skins }: ItemsProps) => {
  return (
    <div className="w-full bg-[#0b0d12] px-20 py-6 flex flex-col gap-6">
      <div className="py-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
        ...
        {skins.map((skin) => (
          <div key={skin.id} className="bg-[#141720] border border-zinc-800/60 rounded-xl overflow-hidden flex flex-col relative group hover:border-zinc-700 transition duration-300">
            ...
            <div className="w-full h-40 bg-zinc-900/40 relative flex items-center justify-center p-4 overflow-hidden">
              ...
              <div className={`absolute bottom-0 inset-x-0 h-[2px] bg-gradient-to-r to-transparent ${
               skin.rarity === 'Contraband' ? 'from-amber-500' : // Contrabando (Laranja/Ouro)
               skin.rarity === 'Covert' ? 'from-red-500' :       // Vermelho
               skin.rarity === 'Classified' ? 'from-pink-500' :   // Rosa
               skin.rarity === 'Restricted' ? 'from-purple-500' : // Roxo
               'from-blue-500'                                    // Mil-Spec (Azul)
              }`}></div>
            </div>
            ...
            <div className="p-4 flex flex-col gap-1.5 flex-1 justify-between">
              <div>
                ...
                <h3 className="text-zinc-400 text-xs font-medium tracking-wide">{skin.weapon}</h3>
                <h2 className="text-white font-bold text-sm tracking-wide truncate">{skin.name}</h2>
              </div>
              ...
              <div className="flex items-center justify-between mt-3 pt-2 border-t border-zinc-800/40">
                <span className="text-white font-bold text-base tracking-wide">${skin.price.toFixed(2)}</span>
                <button className="flex items-center gap-1.5 px-3 py-1.5 bg-sky-500 hover:bg-sky-600 text-white font-bold text-[10px] rounded uppercase transition">Buy</button>
              </div>
            </div>
          </div>
        ))}

        {/* Se a pesquisa não achar nada */}
        {skins.length === 0 && (
          <p className="text-zinc-500 text-sm col-span-full text-center py-10">Nenhuma skin encontrada para a sua busca.</p>
        )}
